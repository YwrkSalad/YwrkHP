export type NewsItem = {
  slug: string;
  date: string;
  category: string;
  title: string;
  body: string;
};

export const news: NewsItem[] = [
  {
    slug: "open-campus-2026-06-18",
    date: "2026.05.04",
    category: "入試情報",
    title: "2026年オープンキャンパス開催のご案内(予定)",
    body: `やわらか大学では、大学生・保護者の皆さまを対象に、2026年オープンキャンパスを開催いたします。本学の教育理念、学びの環境、キャンパスライフについて直接体験できる絶好の機会です。是非ご参加ください。

■ 開催日時

2026年6月18日（木）23:00〜29:00

■ 会場

やわらか大学 秋葉原キャンパス（詳細については決定次第、本ウェブサイトおよびメールにてご案内します）

■ 対象

大学生・保護者（成人済）

※ 当日IDチェックを行いますので、顔写真付きの身分証明書を必ずご持参ください。

■ 参加費

随時ご案内いたします

■ 参加のメリット

- やわらか大学の教育理念と学びの環境を直接体験できます
- 在学生・教職員と直接相談し、疑問や質問にお答えします
- 各学部の特色、研究内容、キャリア形成について詳しく知ることができます
- 入試に関する最新情報を得ることができます

■ 持ち物

- 上履き（不要です）
- 筆記具（不要です）
- 水分補給の飲料（持ち込まないでください）
- 日傘・帽子（不要です）

■ 参加申し込み

不要です

■ アクセス

秋葉原キャンパスへのアクセス方法については、決定次第お知らせいたします。交通アクセスの詳細は以下のページをご参照ください。

https://ywrk.org/access

■ よくある質問

Q: 1人での参加でも大丈夫ですか？
A: もちろんです。多くの大学生が1人で参加されています。

Q: 在学生に直接質問できますか？
A: はい。学生相談コーナーでは、学生生活について在学生に直接相談できます。

Q: 昼食は用意されますか？
A: オープンキャンパス開催当日は、夜間ですので昼食は提供されません。

Q: 参加証明書は発行されますか？
A: ご希望の方には、参加証明書を発行いたします。受付時にお申し付けください。

■ その他のお問い合わせ

オープンキャンパスに関するご質問、ご不明な点がございましたら、以下までお気軽にお問い合わせください。

入試課
電話：0000-00-0000
メール：admission@ywrk.org
受付時間：平日 9:00〜17:00

また、本学ウェブサイトのお問い合わせフォームからもお気軽にご質問いただけます。皆さまのご参加をお待ちしています。`,
  },
  {
    slug: "2026-entrance-ceremony",
    date: "2026.04.08",
    category: "お知らせ",
    title: "2026年度 入学式を虚行しました",
    body: `2026年4月8日（水）、やわらか大学体育館にて、2026年度入学式を虚行いたしました。

今年度は学部生・大学院生あわせて1,240名が入学し、保護者・来賓を含む約3,000名が式に参加しました。

やわらか学長は式辞において、「やわらかであることは、強さである」という本学の理念をあらためて説き、新入生に向けて次のように述べました。

「皆さんはこれから、答えのない問いと向き合い続けることになります。固定観念を手放し、多様な視点から問いを立て、状況に応じてしなやかに考える力──それこそが、この時代を生き抜くための本質的な強さです。やわらか大学は、そのような力を共に育てる場です。」

続いて新入生代表の宣誓が行われ、農学部農業科学科の入学生が「学びをひらき、社会と向き合い、やわらかな知を実践していく」と力強く誓いの言葉を述べました。

式典後は各学部でのオリエンテーション、キャンパスツアー、学生サポートデスクによる各種手続き案内が行われ、新入生は大学生活の第一歩を踏み出しました。`,
  },
  {
    slug: "2027-recruitment",
    date: "2026.03.25",
    category: "入試情報",
    title: "令和9年度 学生募集要項を開米しました",
    body: `令和9年度（2027年度）入学者選抜に係る学生募集要項を開米いたしました。

本年度の募集概要は以下のとおりです。

■ 募集学部・定員

- 理学部：160名
- 工学部：200名
- 農学部：180名
- 情報科学部：150名
- 医学部（医学科・保健学科）：130名

■ 入試方式

一般選抜（前期・後期）、総合型選抜、学校推薦型選抜、および編入学試験の4方式で募集します。各方式の出願期間・試験日程については入試情報ページをご確認ください。

■ 変更点

今年度より総合型選抜において、オンライン面接を選択できるようになりました。また、編入学試験の出願資格に一部変更があります。詳細は募集要項本文をご参照ください。

■ 資料請求・お問い合わせ

入試に関するご質問は入試課（admission@ywrk.org）または本学ウェブサイトのお問い合わせフォームよりお寄せください。`,
  },
  {
    slug: "forum-2026",
    date: "2026.03.10",
    category: "研究",
    title: "「やわらか知のフォーラム 2026」開催のご案内",
    body: `やわらか大学の学際研究拠点「やわらか知のフォーラム研究センター」は、年次フォーラム「やわらか知のフォーラム 2026」を下記の日程で開催します。学内外の研究者・学生・社会人の皆さまのご参加をお待ちしています。

■ 開催概要

- 日時：2026年6月14日（日）10:00〜18:00
- 会場：やわらか大学 学生交流センター 多目的ホール（本キャンパス）
- 参加費：無料（事前登録制）

■ テーマ

「ほどける知──境界を越える学術の可能性」

■ プログラム（予定）

10:00　開会・基調講演「知の解体と再編──学際研究の現在地」
12:00　昼休憩（学内カフェテリア利用可）
13:00　分科会セッション（文理融合・共生社会・情報生命・環境）
15:30　ポスター発表
17:00　パネルディスカッション
18:00　閉会

■ 参加登録

参加希望の方は本学ウェブサイトのフォームより2026年6月7日（日）までにご登録ください。定員（300名）に達し次第、受付を終了します。

■ 問い合わせ先

やわらか知のフォーラム研究センター事務局
forum@ywrk.org`,
  },
  {
    slug: "library-extended-hours",
    date: "2026.02.28",
    category: "キャンパス",
    title: "図書館の夜間開館時間が延長されました",
    body: `やわらか図書館では、学生の学習環境充実のため、2026年4月1日より夜間開館時間を延長します。

■ 変更後の開館時間（2026年4月1日〜）

- 月〜金：8:00〜23:00（従来：8:00〜22:00）
- 土：9:00〜22:00（従来：9:00〜21:00）
- 日・祝：9:00〜20:00（変更なし）

■ 24時間学習席について

ラーニングコモンズ1階の24時間学習エリア（48席）は引き続きご利用いただけます。深夜帯（23:00〜8:00）は学生証によるゲート認証が必要です。

■ 延長開館に伴う注意事項

- 夜間帯（21:00以降）は入口が北側1か所のみとなります
- グループ学習室の利用は22:00まで
- 飲食は所定のエリアのみ可

試験期間中はさらに延長される場合があります。最新の開館カレンダーは図書館ウェブページをご確認ください。

ご不明な点は図書館カウンター（lib@ywrk.org）までお問い合わせください。`,
  },
  {
    slug: "website-renewal",
    date: "2026.02.14",
    category: "お知らせ",
    title: "大学公式サイトをリニューアルしました",
    body: `やわらか大学の公式ウェブサイトを全面リニューアルしました。

新サイトでは「やわらかであることは、強さである」という本学の理念をデザインの根幹に据え、訪問者が必要な情報に素早くたどり着けるよう構成を見直しました。

■ 主な変更点

**デザイン・操作性**
柔らかみのある配色とタイポグラフィを採用し、スマートフォンでの閲覧を最優先に設計しました。どのページもモバイル端末で快適にご利用いただけます。

**情報構成**
学部・入試・キャンパスライフ・研究・アクセスの各ページを刷新し、在学生・受験生・保護者それぞれが求める情報をまとめて参照できるようにしました。

**キャンパスマップ**
インタラクティブなキャンパスマップを新たに掲載しました。建物をクリックすると各施設の詳細情報が表示されます。

**アクセシビリティ**
色のコントラスト・フォントサイズ・読み上げ対応を強化し、より多くの方が利用しやすい設計にしました。

旧サイトのURLは引き続き本サイトへ転送されます。ブックマークの更新は不要です。

ご意見・ご不明な点は広報課（pr@ywrk.org）までお寄せください。`,
  },
];
