  ...
  return (
    <div className="rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
      <div className="mb-5 flex items-center justify-between">
        <div>
          <p className="text-xs font-medium tracking-[0.25em] text-stone-400 uppercase">Page Views</p>
      ...
      <div className="relative">
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart
            ...
            onMouseMove={(state: any) => {
              if (state.isTooltipActive && state.activePayload?.length) {
                setHover({ label: state.activeLabel, count: state.activePayload[0].value });
              } else {
                setHover(null);
              }
            }}
          ...
          >
            ...
            {hover && (
              <ReferenceLine x={hover.label} stroke="#6366f1" strokeWidth={1} strokeDasharray="4 4" />
            )}
        ...
        {hover && (
          <div className="pointer-events-none absolute right-2 top-2 rounded-lg bg-stone-900 px-3 py-2 text-xs text-stone-50 shadow-lg">
            <p className="text-stone-400">{hover.label}</p>
